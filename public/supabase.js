...
// !!! IMPORTANT: Replace with your CURRENT anon key from Supabase Dashboard !!!
...
// If the above key doesn't work, get a new one from:
...
// ============================================
// PREDEFINED COURSES (Fallback if API fails)
// ============================================
const PREDEFINED_COURSES = [
    { id: 1, name: 'LEARNER HUB', description: 'Complete driver training for new drivers covering fundamental rules, vehicle controls, observation, speed management, parking, and NTSA exam preparation.', price: 2999, duration: '8 weeks', icon: 'fa-car', lessons_count: 21, category: 'learner' },
    { id: 2, name: 'PSV PROFESSIONAL COURSE', description: 'Public Service Vehicle certification with passenger management, customer care, safety protocols, NTSA compliance, and conductor professionalism.', price: 3999, duration: '8 weeks', icon: 'fa-bus', lessons_count: 16, category: 'psv' },
    { id: 3, name: 'EV DRIVER & RIDER COURSE', description: 'Electric vehicle operations, charging safety, battery management, regenerative braking, and eco-driving techniques for EVs.', price: 1999, duration: '5 weeks', icon: 'fa-car-battery', lessons_count: 10, category: 'ev' },
    { id: 4, name: 'DRIVER REFRESHER COURSE', description: 'Advanced defensive driving, skill enhancement, hazard perception, professional driver excellence, and breaking bad habits.', price: 1499, duration: '4 weeks', icon: 'fa-sync-alt', lessons_count: 8, category: 'refresher' },
    { id: 5, name: 'BODA BODA SAFETY COURSE', description: 'Professional motorcycle rider training with PPE, defensive riding, passenger safety, road etiquette, and accident prevention.', price: 2499, duration: '6 weeks', icon: 'fa-motorcycle', lessons_count: 26, category: 'boda' },
    { id: 6, name: 'SCHOOL BUS/VAN DRIVER COURSE', description: 'Specialized training for school transport drivers focusing on child safety, boarding/alighting procedures, and emergency response.', price: 2999, duration: '7 weeks', icon: 'fa-school', lessons_count: 7, category: 'school' },
    { id: 7, name: 'DEFENSIVE DRIVER COURSE', description: 'Master defensive driving techniques including hazard perception, risk management, space cushion driving, and collision avoidance.', price: 2499, duration: '6 weeks', icon: 'fa-shield-alt', lessons_count: 30, category: 'defensive' },
    { id: 8, name: 'E-ROAD SAFETY LIBRARY & QUIZ BANK', description: '1000+ NTSA-style exam questions covering road signs, highway code, defensive driving, traffic rules, and professional conduct.', price: 999, duration: 'Self-paced', icon: 'fa-question-circle', lessons_count: 15, category: 'quiz' }
];
...
async function getAllCourses() {
    try {
        const { data, error } = await supabase
            ...
            .eq('is_active', true);
        if (error) throw error;
        return data && data.length > 0 ? data : PREDEFINED_COURSES;
    } catch (error) {
...
async function getAllQuizQuestions() {
    try {
        const { data, error } = await supabase
            ...
            .select('*');
        if (error) throw error;
        return data && data.length > 0 ? data : PREDEFINED_QUIZ;
    } catch (error) {
        console.log('Using predefined quiz questions');
...
console.log('✅ MEI DRIVE AFRICA - Ready (with fallback data)');
